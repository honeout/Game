...
void Sprite::RenderCircle(ID3D11DeviceContext* context, const DirectX::XMFLOAT2& center, float radius, const DirectX::XMFLOAT2& scale, float angle, const DirectX::XMFLOAT4& color, int n) const
{
    ...
    for (int i = 0; i <= n; i++)
    {
        ...
        vertex->position.x = center.x + rx * cosValue - ry * sinValue;
        vertex->position.y = center.y + rx * sinValue + ry * cosValue;

        vertex->position.x = -1.0f + vertex->position.x * 2 / viewport.Width;

        vertex->position.y = 1.0f - vertex->position.y * 2 / viewport.Height;
